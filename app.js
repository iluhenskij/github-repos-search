class Search {
   constructor() {
      ...
      this.pagePlus = document.querySelector(".search_num_plus");
      this.pageMinus = document.querySelector(".search_num_minus");
      ...
      this.searchLine.addEventListener("keypress",(event)=>{
         if (event.key === 'Enter') {
            this.searchRepositories.call(this);
            console.log(this);
            console.log(event);

         }
      } );
      this.pagePlus.addEventListener("click",()=>{
         if(+this.searchPage.value>98)return;
         this.searchPage.value = +(this.searchPage.value) + 1
      })
      this.pageMinus.addEventListener("click",()=>{
         if(+this.searchPage.value<2)return
         this.searchPage.value = +(this.searchPage.value) - 1
      })
      this.searchPage.addEventListener("change",this.searchRepositories())
   }
          ...
          .then(res => {
             new Promise(resolve =>{
                ...
                outer: for (let i = 0; i < this.resultsPerPage; i++) {
                   ...
                   this.resultLines.children[i].children[4].children[0].innerText = "Go to GitHub Page";
                }
                resolve(res)
             })
          })
